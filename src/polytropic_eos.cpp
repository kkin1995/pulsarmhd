...
PolytropicEOSData PolytropicEOS::getEOSParameters(PolytropicGasType type) const {
  ...
  case PolytropicGasType::ELECTRON_NON_RELATIVISTIC:
    return PolytropicEOSData("electron_non_relativistic",
                             1.0036e13 / std::pow(mu_e, 5.0 / 3.0), // k
                             5.0 / 3.0,                             // gamma
                             mu_e);

  case PolytropicGasType::ELECTRON_RELATIVISTIC:
    return PolytropicEOSData("electron_relativistic", 1.2435e15 / std::pow(mu_e, 4.0 / 3.0), // k
                             4.0 / 3.0, // gamma
                             mu_e);

  case PolytropicGasType::NEUTRON_NON_RELATIVISTIC:
    return PolytropicEOSData("neutron_non_relativistic",
                             5.3802e9,  // k
                             5.0 / 3.0, // gamma
  ...
  case PolytropicGasType::NEUTRON_RELATIVISTIC:
    return PolytropicEOSData("neutron_relativistic",
                             1.2293e15, // k
                             4.0 / 3.0, // gamma
                             1.0        // Not applicable for neutrons
    );
...
bool PolytropicEOS::generateEOSTable(const PolytropicEOSParameters &params) const {
  try {
    ...
    std::vector<double> densities = createDensityGrid(params.rho_min, params.rho_max,
                                                      params.num_points, params.use_log_spacing);
    ...
    // Write to file
    bool success = writeEOSToFile(params.output_file, densities, pressures, eos_data,
...
// Create density grid
std::vector<double> PolytropicEOS::createDensityGrid(double rho_min, double rho_max, int num_points,
