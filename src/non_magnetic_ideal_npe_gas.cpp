...
NonMagneticNPEGas::NonMagneticNPEGas(bool debug)
    : debug_mode(debug),
      ...
      lambda_e(hbar / (m_electron * c)), lambda_p(hbar / (m_proton * c)),
      lambda_n(hbar / (m_neutron * c)) {}

// Helper Functions
double NonMagneticNPEGas::phi(double x) const {
  if (x < 0.1) {
    return (1.0 / (15 * pow(M_PI, 2.0))) *
           (pow(x, 5.0) - (5.0 / 14.0) * pow(x, 7.0) + (5.0 / 24.0) * pow(x, 9.0));
  }
  if (x > 10.0) {
  ...
  double second_factor =
      x * (pow(1.0 + pow(x, 2.0), 1.0 / 2.0)) * ((2.0 / 3.0) * pow(x, 2.0) - 1.0) +
...
double NonMagneticNPEGas::chi(double x) const {
  if (x < 0.1) {
    return (1.0 / (3.0 * pow(M_PI, 2.0))) *
           (pow(x, 3.0) + (3.0 / 10.0) * pow(x, 5.0) - (3.0 / 56.0) * pow(x, 7.0));
  }
  if (x > 10.0) {
    return (1.0 / (4.0 * pow(M_PI, 2.0))) *
           (pow(x, 4.0) + pow(x, 2.0) - (1.0 / 2.0) * log(2.0 * x));
  }
  double first_factor = 1 / (8.0 * pow(M_PI, 2.0));
  double second_factor = x * pow((1 + pow(x, 2.0)), 1.0 / 2.0) * (1.0 + 2.0 * pow(x, 2.0)) -
                         std::log(x + pow((1.0 + pow(x, 2.0)), 1.0 / 2.0));

  return first_factor * second_factor;
}

double NonMagneticNPEGas::interpolate_spline(double t, const std::vector<double> &x,
                                             const std::vector<double> &y) const {
  // Ensure input sizes match
  if (x.size() != y.size()) {
    std::cerr << "Error: x and y arrays must have the same size for spline interpolation." << '\n';
    return NAN;
  }
...
void NonMagneticNPEGas::BetaEquilibriumSolver::f_and_df(double x_n, void *p, double *f_value,
                                                        double *df_value) {
  *f_value = f(x_n, p);
  *df_value = f_prime(x_n, p);
}

double NonMagneticNPEGas::BetaEquilibriumSolver::solve(double x_n, double n_B, double m_e,
                                                       double m_p, double m_n, double x_p,
  ...
  if (debug) {
    std::cout << "Initial: f(x_lo) = " << f_lo << ", f(x_hi) = " << f_hi << '\n';
  }
  ...
  if (debug) {
    std::cout << "x_lo = " << x_lo << " | f(x_lo) = " << f(x_lo, &params) << '\n';
    std::cout << "x_hi = " << x_hi << " | f(x_hi) = " << f(x_hi, &params) << '\n';
  }

  // Check if we found straddling points
  if (f_lo * f_hi >= 0) {
    std::cerr << "Could not find straddling points for root." << '\n';
  ...
  do {
    ...
    if (debug) {
      if (iter % 10 == 0) {
        std::cout << "Iter " << iter << ": x = " << x << ", f(x) = " << f(x, &params) << '\n';
      }
    }
  ...
  if (status == GSL_SUCCESS && debug) {
    std::cout << "Succeeded for n_B = " << n_B << '\n';
  } else {
    std::cerr << "Failed for n_B = " << n_B << ". Skipping..." << '\n';
  }

  if (status != GSL_CONTINUE && iter >= max_iter && debug) {
    std::cerr << "Exceeded max iterations. Skipping n_B = " << n_B << "." << '\n';
    return 1;
  }
...
void NonMagneticNPEGas::calculateEOS(const std::string &filename, double nB_min, double nB_max,
                                     int num_points) {
  double x_n, x_e, x_p;
  ...
  for (double n_B : n_B_list) {
    ...
    // 3. Transition Region
      ...
      if (!found_high_boundary) {
        ...
        x_p = (m_electron / m_proton) * x_e;
        x_n = pow((3.0 * pow(M_PI, 2.0) * pow(lambda_n, 3.0) * n_B_upper) / (1.0 + 0.0026),
                  1.0 / 3.0);

        x_n = BetaEquilibriumSolver::solve(x_n, n_B_upper, m_electron, m_proton, m_neutron, x_p,
                                           x_e, debug_mode);
        if (x_n == 1) {
          std::cerr << "Fallback: High boundary values for n_B = " << n_B_upper << '\n';
          x_e = xe_min * pow(n_B_upper / n_B_threshold, 1.0 / 3.0);
          x_p = (m_electron / m_proton) * x_e;
          x_n = pow((3.0 * pow(M_PI, 2.0) * pow(lambda_n, 3.0) * n_B_upper) / (1.0 + 0.0026),
                    1.0 / 3.0);
      ...
      if (found_low_boundary && found_high_boundary) {
        if (debug_mode) {
          std::cout << "Low boundary: x_e = " << last_low_xe << ", x_p = " << last_low_xp
                    << ", x_n = " << last_low_xn << '\n';
          std::cout << "High boundary: x_e = " << first_high_xe << ", x_p = " << first_high_xp
                    << ", x_n = " << first_high_xn << '\n';
        }
        ...
        for (int j = 0; j <= num_spline_points; ++j) {
          const double t = static_cast<double>(j) / +static_cast<double>(num_spline_points);
      ...
      if (debug_mode) {
        std::cout << "Transition Region - Appearence of Neutrons" << '\n';
        std::cout << "x_n = " << x_n << '\n';
        std::cout << "x_e = " << x_e << '\n';
        std::cout << "x_p = " << x_p << '\n';
      }
    ...
    // 4. High Density Regime
      ...
      x_n = BetaEquilibriumSolver::solve(x_n, n_B, m_electron, m_proton, m_neutron, x_p, x_e,
                                         debug_mode);
      if (x_n == 1) {
        std::cerr << "Beta equilibrium solver failed in high density region at n_B = " << n_B
                  << '\n';
        continue;
      }

      if (debug_mode) {
        std::cout << "Neutron Rich Region" << '\n';
        std::cout << "x_n = " << x_n << '\n';
        std::cout << "x_e = " << x_e << '\n';
        std::cout << "x_p = " << x_p << '\n';
      }
    ...
    if (P < 0 || rho < 0 || n < 0) {
      std::cerr << "Warning: Negative values detected. n_B = " << n_B << ", P = " << P
                << ", rho = " << rho << ", n = " << n << '\n';
    }
