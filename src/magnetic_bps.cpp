...
EquilibriumComposition MagneticBPSEOS::computeEquilibriumComposition(double nB) {
    ...
    for (int i = 0; i < (int) atomic_masses_.size(); ++i) {
        ...
        if (B_ratio_electron_ <= 0.0) {
            ...
            const double x = pow(3.0 * M_PI * M_PI * pow(lambda_e,3.0) * calculated_electron_density, 1.0/3.0);
...
std::vector<EquilibriumComposition> MagneticBPSEOS::runSimulation(double nB_min, double nB_max, int num_points) {
    ...
    #pragma omp parallel for schedule(dynamic, 2) reduction(+:sum_iters,sum_nu) reduction(max:max_iters,max_nu)
    for (int i = 0; i < num_points; ++i) {
        ...
        if (comp.iterations > max_iters) max_iters = comp.iterations;
        if (comp.max_landau_level > max_nu) max_nu = comp.max_landau_level;
        ...
        if (done % report_step == 0 || done == num_points) {
            ...
            #endif
            {
                ...
                std::cout << "[progress] " << done << "/" << num_points
    ...
    std::cout << "[timing] runSimulation took " << (t1 - t0)
              << " s with " << omp_get_max_threads() << " threads\n"
              << "[stats] iterations avg=" << avg_iters
              << " max=" << max_iters
              << " | nu_max avg=" << avg_nu
              << " max=" << max_nu << "\n";
    #endif
