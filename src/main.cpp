...
{
    log_rho.clear(); log_P.clear(); epsilon.clear();
    ...
    if (i_eps < 0) i_eps = find_col("energy_density");
    if (i_eps < 0) i_eps = find_col("eps");
    ...
    while (std::getline(file, line)) {
        ...
        try {
            ...
            if (!std::isfinite(lr) || !std::isfinite(lp)) continue;
            ...
            if (i_eps >= 0 && (size_t)i_eps < toks.size()) {
                ...
                if (std::isfinite(e)) epsilon.push_back(e);
                else epsilon.push_back(std::numeric_limits<double>::quiet_NaN());
            }
        } catch (...) { continue; }
    }
    file.close();
...
int main() {
    ...
    if (readEoSData(eos_filename, log_rho, log_P, epsilon)) {
        ...
        struct Row { double lp, lr, eps; bool has_eps; };
        std::vector<Row> rows;
        rows.reserve(log_P.size());
        for (size_t i = 0; i < log_P.size(); ++i) {
            Row r{ log_P[i], log_rho[i], 0.0, false };
            if (!epsilon.empty() && i < epsilon.size()) { r.eps = epsilon[i]; r.has_eps = true; }
            rows.push_back(r);
        }
        ...
        auto push_row = [&](const Row& r){
            lp2.push_back(r.lp);
            lr2.push_back(r.lr);
            if (!epsilon.empty()) eps2.push_back(r.has_eps ? r.eps : std::numeric_limits<double>::quiet_NaN());
        };
        if (!rows.empty()) push_row(rows.front());
        for (size_t i = 1; i < rows.size(); ++i) {
            if (rows[i].lp > lp2.back()) push_row(rows[i]);  // strict >
        }

        log_P.swap(lp2);
        log_rho.swap(lr2);
        if (!epsilon.empty()) epsilon.swap(eps2);
        ...
        if (log_rho.size() < 2) {
            std::cerr << "[ScanBuild] EOS has < 2 points after cleaning (size="
                    << log_rho.size() << "). Cannot build rho_c scan.\n";
            return 1;
        }
        ...
        if (!std::isfinite(lr_min) || !std::isfinite(lr_max)) {
            std::cerr << "[ScanBuild] Non-finite log_rho bounds: "
                    << lr_min << ", " << lr_max << "\n";
            return 1;
        }
        ...
        if (!(scan_hi > scan_lo)) {
            std::cerr << "[ScanBuild] scan_hi <= scan_lo after clamping: "
                    << scan_lo << " .. " << scan_hi << "\n";
            return 1;
        }
        ...
        if (!std::isfinite(log_start) || !std::isfinite(log_end)) {
            std::cerr << "[ScanBuild] Non-finite log bounds: "
                    << log_start << ", " << log_end << "\n";
            return 1;
        }
        ...
        std::cout << "[ScanBuild] rho_c scan: [" << central_densities.front() << ", "
                << central_densities.back() << "] g/cm^3 over " << num_points
                << " points.\n";
    } else {
    ...
    if (log_rho.empty() || log_P.empty() || log_rho.size() != log_P.size()) {
        std::cerr << "[EoS] Bad vectors: log_rho=" << log_rho.size()
            << " log_P=" << log_P.size() << "\n";
        return 1;
    }
    ...
    if (!epsilon.empty() && epsilon.size() == log_P.size()) {
        ...
        if (gsl_spline_init(spline_eps, log_P.data(), epsilon.data(), log_P.size()) != GSL_SUCCESS) {
            ...
            gsl_spline_free(spline_eps);  spline_eps = nullptr;
            gsl_interp_accel_free(acc_eps); acc_eps = nullptr;
        }
    }

    // --- choose surface and compute P_stop using inverse spline
    const double rho_surface = 1e6;
    const double goal_log_rho = std::clamp(std::log10(rho_surface), log_rho.front()+1e-9, log_rho.back()-1e-9);
    ...
    for (size_t i = 0; i < central_densities.size(); ++i) {
        ...
        oss << "data/tov_solution_magnetic_bps_bbp_polytrope_" << std::scientific << std::setprecision(2) << rho_c << ".csv";
        ...
        TovResult res = non_rotating_stellar_structure_spline(
            ...
            base_dlogr, true,
            P_stop, filename,
            spline_eps, acc_eps            // <-- pass ε-spline (may be nullptr)
        );
    ...
    // Clean up
    if (spline_eps) gsl_spline_free(spline_eps);
    if (acc_eps)    gsl_interp_accel_free(acc_eps);
