...
protected:
  ...
  bool compareDerivatives(const std::vector<double> &result1, const std::vector<double> &result2,
                          double tolerance) {
    ...
    for (size_t i = 0; i < result1.size(); i++) {
      double rel_error =
          std::abs(result1[i] - result2[i]) / (std::abs(result1[i]) + std::abs(result2[i]) + 1e-20);
...
TEST_F(StellarStructureTest, TOVDerivativesLimitsToNewtonian) {
  ...
  double pressure_rel_diff =
      std::abs(tov_result[1] - newton_result[1]) / std::abs(newton_result[1]);
...
TEST_F(StellarStructureTest, TOVDerivativesSplineVsPolytropic) {
  ...
  auto polytropic_result = tolman_oppenheimer_volkoff_derivatives(log_r, state, k, gamma);

  // Results should be similar (within reasonable tolerance) since test EOS is polytropic
  double mass_rel_diff =
      std::abs(spline_result[0] - polytropic_result[0]) / std::abs(polytropic_result[0]);
  double pressure_rel_diff =
      std::abs(spline_result[1] - polytropic_result[1]) / std::abs(polytropic_result[1]);
...
TEST_F(StellarStructureTest, NewtonianDerivativesEOSConsistency) {
  ...
  EXPECT_LT(relative_error, numerical_tolerance)
      << "EOS relation P = k*rho^gamma should be satisfied";
}
...
TEST_F(StellarStructureTest, AllEOSTypesSupported) {
  ...
  std::vector<PolytropicGasType> eos_types = {
      PolytropicGasType::ELECTRON_NON_RELATIVISTIC, PolytropicGasType::ELECTRON_RELATIVISTIC,
      PolytropicGasType::NEUTRON_NON_RELATIVISTIC, PolytropicGasType::NEUTRON_RELATIVISTIC};
  ...
  for (size_t i = 0; i < eos_types.size(); i++) {
    EXPECT_NO_THROW({
      auto result =
          non_rotating_stellar_structure(eos_types[i], test_densities[i], 10.0, 1e7, 0.1, 2.0);
      EXPECT_GT(std::get<0>(result), 0) << "EOS type " << i << " should complete integration";
    }) << "EOS type "
       << i << " should work without throwing";
  }
}
...
TEST_F(StellarStructureTest, SplineBasedIntegrationBasicFunctionality) {
  ...
  auto result = non_rotating_stellar_structure_spline(test_spline_inv, test_acc_inv, test_min_logP,
                                                      ...
                                                      base_dlogr, true, 1e-8, output_file);
...
TEST_F(StellarStructureTest, SplineBasedIntegrationErrorHandling) {
  ...
  EXPECT_NO_THROW({
    auto invalid_result = non_rotating_stellar_structure_spline(
        test_spline_inv, test_acc_inv, test_min_logP, test_max_logP, rho_c, 1000.0, 100.0, 0.01,
        true, 1e-8); // r_end < r_start
  ...
  auto result_small_step =
      non_rotating_stellar_structure_spline(test_spline_inv, test_acc_inv, test_min_logP,
                                            test_max_logP, rho_c, 1.0, 1e5, 0.001, true, 1e-8);
...
TEST_F(StellarStructureTest, SplineVsPolytropicComparison) {
  ...
  auto spline_result = non_rotating_stellar_structure_spline(
      ...
      false, 1e-8); // Disable adaptive for fair comparison

  // Use equivalent polytropic solver
  PolytropicGasType eos_type = PolytropicGasType::NEUTRON_RELATIVISTIC;
  auto polytropic_result =
      non_rotating_stellar_structure(eos_type, rho_c, r_start, r_end, dlogr, 2.0);
...
TEST_F(StellarStructureTest, SplineIntegrationFileOutput) {
  ...
  auto result = non_rotating_stellar_structure_spline(test_spline_inv, test_acc_inv, test_min_logP,
                                                      test_max_logP, rho_c, 1.0, 1e6, 0.02, true,
                                                      1e-8, output_file);
  ...
  EXPECT_EQ(line_count - 1, std::get<0>(result))
      << "Number of data lines should match integration steps";
}
