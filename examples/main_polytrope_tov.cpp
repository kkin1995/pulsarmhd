 ...
 */
void writeMassRadiusSummary(const std::string &filename, const std::vector<double> &densities,
  ...
  // Write header
  file << "central_density[g/"
          "cm^3],mass[M_sun],radius[km],log_central_density,log_mass[g],log_radius[cm]"
       << std::endl;

  // Write data
  for (size_t i = 0; i < densities.size(); ++i) {
    if (i < masses.size() && i < radii.size()) {
      file << std::scientific << std::setprecision(6) << densities[i] << "," << masses[i] << ","
           << radii[i] << "," << std::log10(densities[i]) << "," << std::log10(masses[i] * M_SUN)
 ...
 */
double findRadiusForMass(double target_mass, const std::vector<double> &masses,
                         const std::vector<double> &radii, const std::vector<double> &densities) {
  ...
  for (size_t i = 0; i < masses.size(); ++i) {
    if (std::abs(masses[i] - target_mass) < 1e-6) {
      ...
      std::cout << "  Mass: " << std::fixed << std::setprecision(3) << masses[i] << " M☉"
                << std::endl;
      std::cout << "  Radius: " << std::setprecision(2) << radii[i] << " km" << std::endl;
      std::cout << "  Central density: " << std::scientific << std::setprecision(2) << densities[i]
                << " g/cm³" << std::endl;
      return radii[i];
    }
  }
  ...
  if (closest_lower_mass >= 0 && closest_upper_mass >= 0 &&
    ...
    double fraction =
        (target_mass - closest_lower_mass) / (closest_upper_mass - closest_lower_mass);
    double interpolated_radius =
        closest_lower_radius + fraction * (closest_upper_radius - closest_lower_radius);
    double interpolated_density =
        closest_lower_density * std::pow(closest_upper_density / closest_lower_density, fraction);

    std::cout << "  Interpolated result:" << std::endl;
    std::cout << "  Target mass: " << std::fixed << std::setprecision(3) << target_mass << " M☉"
              << std::endl;
    std::cout << "  Interpolated radius: " << std::setprecision(2) << interpolated_radius << " km"
              << std::endl;
    ...
    std::cout << "    Upper: " << std::setprecision(3) << closest_upper_mass << " M☉ → "
              << std::setprecision(2) << closest_upper_radius << " km" << std::endl;
  ...
  if (closest_lower_mass >= 0 && closest_upper_mass < 0) {
    std::cout << "  Target mass (" << std::fixed << std::setprecision(3) << target_mass
              << " M☉) is above our maximum calculated mass (" << std::setprecision(3)
              << closest_lower_mass << " M☉)" << std::endl;
  } else if (closest_lower_mass < 0 && closest_upper_mass >= 0) {
    std::cout << "  Target mass (" << std::fixed << std::setprecision(3) << target_mass
              << " M☉) is below our minimum calculated mass (" << std::setprecision(3)
              << closest_upper_mass << " M☉)" << std::endl;
  }
...
void displayHeader(const EOSConfig &config) {
  ...
  std::cout << "  Polytropic index (γ): " << std::fixed << std::setprecision(5) << eos_data.gamma
            << std::endl;
  std::cout << "  Proportionality constant (k): " << std::scientific << std::setprecision(6)
            << eos_data.k << std::endl;
  if (config.type == PolytropicGasType::ELECTRON_NON_RELATIVISTIC ||
      config.type == PolytropicGasType::ELECTRON_RELATIVISTIC) {
    std::cout << "  Mean molecular weight (μₑ): " << std::fixed << std::setprecision(1)
              << config.mu_e << std::endl;
  }
  std::cout << std::endl;
}
...
int main() {
  ...
  std::cout << "  Maximum radius: " << config.r_end << " cm (" << config.r_end / KM_TO_CM << " km)"
            << std::endl;
  ...
  std::vector<double> central_densities =
      generateCentralDensities(config.rho_min, config.rho_max, config.num_densities);
  ...
  for (size_t i = 0; i < central_densities.size(); ++i) {
    ...
    std::cout << std::fixed << std::setprecision(0) << "(" << (i + 1) << "/"
              << central_densities.size() << ") " << std::scientific << std::setprecision(2)
              << rho_c << " g/cm³ -> ";

    try {
      // Solve TOV equations
      auto result = non_rotating_stellar_structure(config.type, rho_c, r_start, config.r_end, dlogr,
                                                   config.mu_e);
      ...
      std::cout << std::fixed << std::setprecision(0) << steps << " steps, " << std::setprecision(3)
                << mass_solar << " M☉, " << std::setprecision(1) << radius_km << " km" << std::endl;
  ...
  if (!successful_densities.empty()) {
    ...
    std::cout << "  Mass range: " << std::fixed << std::setprecision(3) << min_mass << " - "
              << max_mass << " M☉" << std::endl;
    std::cout << "  Radius range: " << std::setprecision(1) << min_radius << " - " << max_radius
              << " km" << std::endl;
    ...
    if (config.type == PolytropicGasType::NEUTRON_RELATIVISTIC ||
      ...
      double radius_077 =
          findRadiusForMass(target_mass, stellar_masses, stellar_radii, successful_densities);

      if (radius_077 > 0) {
        std::cout << std::endl;
        std::cout << "✓ SUCCESS: Found radius for " << std::fixed << std::setprecision(2)
                  << target_mass << " M☉ = " << std::setprecision(2) << radius_077 << " km"
                  << std::endl;
      } else {
        std::cout << std::endl;
