...
void demonstrateBasicUsage() {
  ...
  double pressure_electron =
      calculator.calculatePressure(density, electron_rel.k, electron_rel.gamma);
  double pressure_neutron = calculator.calculatePressure(density, neutron_rel.k, neutron_rel.gamma);

  std::cout << "\nPressure calculations for ρ = " << std::scientific << density
            << " g/cm³:" << std::endl;
...
void demonstrateEOSTableGeneration() {
  ...
  std::cout << "  Density range: " << std::scientific << params.rho_min << " to " << params.rho_max
            << " g/cm³" << std::endl;
...
void compareAllEOSTypes() {
  ...
  std::cout << std::left << std::setw(30) << "Type" << std::setw(15) << "k" << std::setw(10) << "γ"
  ...
  std::cout << "\nPressure comparison at ρ = " << std::scientific << ref_density
            << " g/cm³:" << std::endl;
  ...
  for (const auto &[type, data] : all_types) {
    ...
    std::cout << std::left << std::setw(30) << data.name;
    std::cout << "P = " << std::scientific << std::setprecision(3) << pressure << " dyne/cm²"
              << std::endl;
  }
}
