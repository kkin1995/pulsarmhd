...
struct PolytropicEOSParameters {
  ...
  PolytropicEOSParameters()
      : gas_type(PolytropicGasType::NEUTRON_RELATIVISTIC), rho_min(1e10), rho_max(1e18),
        num_points(1000), output_file("polytropic_eos.csv"), use_log_spacing(true),
 ...
 * @return true if successful, false otherwise
 */
bool calculatePolytropicEOS(const std::string &gas_type_str, const PolytropicEOSParameters &params);
